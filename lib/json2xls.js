...
var transform = function(json,config) {
    var conf = transform.prepareJsonMulti(json,config);
...
//prepare json to be in the correct format for excel-export
transform.prepareJsonMulti = function(json,config) {
    if(config && config.multi == true){
        return json.map(function(innerJson){
            return transform.prepareJson(innerJson,config);
        })
    }else{
        return transform.prepareJson(json,config);
    }
}
